...
function determineLabelingAction(context) {
    ...
    if (
        ...
        ['opened', 'edited', 'synchronize'].includes(payload.action)
    ) {
...
async function labelIssue(octokit, context) {
    ...
    const newLabels = suggestedLabels.filter(
    ...
    await generateIssueLabelingReport(
        octokit,
        context,
        issue,
        suggestedLabels,
...
async function labelPullRequest(octokit, context) {
    ...
    // Generate labeling report
    await generatePRLabelingReport(
        octokit,
        context,
        pr,
        suggestedLabels,
...
async function analyzeIssueForLabels(issue) {
    ...
    if (issue.body) {
        const templateLabelMatch = issue.body.match(
            /(?:labels?|type):\s*([^\r\n]+)/i
        );
...
function findStandardLabel(input) {
    ...
    // Direct matches
    for (const [, config] of Object.entries(LABEL_FAMILIES)) {
...
async function applyLabelsToIssue(octokit, context, issueNumber, labels, type) {
    if (config.dryRun) {
        log(
            `[DRY RUN] Would apply labels to ${type} #${issueNumber}: ${labels.join(', ')}`
        );
        return;
    }
...
function isStandardLabel(labelName) {
    ...
    return (
        Object.values(LABEL_FAMILIES).some((category) =>
...
function generateExpectedLabels() {
    ...
    Object.entries(LABEL_FAMILIES).forEach(([_, config]) => {
            ...
            try {
                ...
                if (error.status === 422) {
                    ...
                    try {
                        ...
                        log(
                            `Error updating label ${label.name}: ${updateError.message}`,
                            'warning'
                        );
                    }
                } else {
                    log(
                        `Error creating label ${label.name}: ${error.message}`,
                        'warning'
                    );
                }
            }
 ...
 */
async function generateIssueLabelingReport(
    octokit,
    context,
    issue,
    suggestedLabels,
...
### Labels Applied:`;
    ...
    if (suggestedLabels.length > newLabels.length) {
        const existingRelevant = suggestedLabels.filter((label) =>
 ...
 */
async function generatePRLabelingReport(
    octokit,
    context,
    pr,
    suggestedLabels,
...
async function generateLabelValidationReport(octokit, context, validation) {
    const totalLabels =
        validation.standardLabels.length + validation.nonStandardLabels.length;
    const standardPercentage = Math.round(
        (validation.standardLabels.length / totalLabels) * 100
    );
...
- ${validation.missingLabels.length === 0 ? '✅' : 'ℹ️'} **Missing recommended labels:** ${validation.missingLabels.length}`;
    ...
    if (
        validation.nonStandardLabels.length === 0 &&
