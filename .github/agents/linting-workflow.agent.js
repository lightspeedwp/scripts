...
async function run() {
    try {
        ...
        const criticalIssues = [
            ...lintingReport.configurationIssues.filter(
                (i) => i.severity === 'error'
            ),
            ...lintingReport.codeQualityIssues.filter(
                (i) => i.severity === 'error'
            ),
        ];
        ...
        if (criticalIssues.length > 0 && !config.dryRun) {
            actionsCore.setFailed(
                `${criticalIssues.length} critical linting issues found`
            );
        }
 ...
 */
async function analyzeLinterConfig(
    octokit,
    context,
    linter,
    linterConfig,
    ...
    try {
        ...
        if (error.status === 404) {
            report.configurationIssues.push({
                ...
                severity:
                    linter === 'shellcheck' || linter === 'markdownlint'
                        ? 'error'
                        : 'warning',
...
async function validateShellCheckConfig(content, report) {
    ...
    const hasExternalSources = lines.some(
        (line) =>
            line.includes('external-sources=true') ||
    ...
    if (!hasExternalSources) {
        report.configurationIssues.push({
            ...
            message:
                'ShellCheck config should include external-sources=true for better analysis',
            severity: 'warning',
    ...
    if (!hasShellSpec) {
        report.configurationIssues.push({
            ...
            message:
                'ShellCheck config should specify shell type (e.g., shell=bash)',
            severity: 'warning',
    ...
    try {
        ...
        if (!config.extends || !Array.isArray(config.extends)) {
            report.configurationIssues.push({
                type: 'eslint_extends',
                message:
                    'ESLint config should extend recommended configurations',
                severity: 'warning',
                suggestion:
                    'Add "extends": ["eslint:recommended"] to .eslintrc.json',
            });
        }
        ...
        if (!config.env) {
            report.configurationIssues.push({
                type: 'eslint_env',
                message:
                    'ESLint config should specify environment (node, browser, es6)',
                severity: 'warning',
    ...
    try {
        ...
        for (const file of workflowFiles) {
            if (file.name.endsWith('.yml') || file.name.endsWith('.yaml')) {
                const workflowAnalysis = await analyzeWorkflowFile(
                    octokit,
                    context,
                    file,
                    report
                ...
                if (workflowAnalysis.hasLinting) {
                    ...
                    workflowAnalysis.linters.forEach((linter) =>
                        lintersCovered.add(linter)
                    );
                }
            }
        }
        ...
        if (!hasLintingWorkflow) {
            report.workflowIssues.push({
                ...
                severity: 'error',
                suggestion:
                    'Create a linting workflow that runs on pull requests',
            });
        }

        // Check coverage of available linters
        const availableLinters = Object.keys(LINTER_CONFIGS);
        const missingLinters = availableLinters.filter(
            (linter) => !lintersCovered.has(linter)
        );
        ...
        if (missingLinters.length > 0) {
            report.workflowIssues.push({
                ...
                severity: 'warning',
                suggestion:
                    'Add missing linters to workflow for complete coverage',
            });
        }
    } catch (error) {
...
async function analyzeWorkflowFile(octokit, context, file, report) {
    ...
    const content = Buffer.from(workflowContent.content, 'base64').toString(
        'utf8'
    );
    const linters = [];
...
async function analyzeFileForLinting(octokit, context, file, report) {
    ...
    try {
        ...
        await performBasicLintingChecks(
            file.filename,
            fileContent,
            applicableLinters,
            report
        ...
        log(
            `Error analyzing file ${file.filename}: ${_error.message}`,
            'warning'
        );
    }
}
...
async function performBasicLintingChecks(filename, content, linters, report) {
    ...
    if (
        ['.js', '.mjs', '.jsx'].includes(extension) &&
        linters.includes('eslint')
    ) {
...
async function performShellScriptChecks(filename, lines, report) {
    ...
    const hasErrorHandling = lines.some(
        (line) => line.includes('set -e') && line.includes('pipefail')
    );
    ...
    const unquotedVarLines = lines
        ...
        .filter(({ line }) => {
            ...
            return (
                /\$[A-Za-z_][A-Za-z0-9_]*(?!\s*[=[)]|\s*\))/u.test(line) &&
                !line.includes('"$') &&
                !line.includes("'$")
            );
        })
...
async function generateAutoFixSuggestions(octokit, context, report) {
    ...
    const autoFixableIssues = report.codeQualityIssues.filter(
...
async function generateLintingReport(octokit, context, report) {
    const totalIssues =
        report.configurationIssues.length +
        report.workflowIssues.length +
        report.codeQualityIssues.length;

    const errorCount = [
        ...report.configurationIssues,
        ...report.workflowIssues,
        ...report.codeQualityIssues,
    ...
    const overallStatus =
        errorCount > 0 ? 'FAILING' : warningCount > 0 ? 'WARNING' : 'PASSING';
