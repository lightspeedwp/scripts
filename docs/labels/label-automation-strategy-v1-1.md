...
## _Includes Changelog Governance & Git Workflow strategies_
...
- After compiling the new release notes, the Action will commit the updated `CHANGELOG.md` to the repository (for example, directly to the `main` branch as part of the release commit). WooCommerce implemented a similar post-release changelog update workflow – _“Changelogs are now auto-updated post-release”_ – meaning as soon as a release is cut, the changelog file is immediately updated with that release’s details. We will mirror this approach to ensure our changelog is always current.
